...
class Candidate:
    ...
    lane: str = "CORE_MARGIN"
    sourcing_route: str = ""
    ebay_validation_url: str = ""
...
def is_aliexpress_url(url: str) -> bool:
    ...
    return any(token in host for token in ALIEXPRESS_DOMAIN_TOKENS)


def first_aliexpress_link(links: list[str]) -> str:
    for link in links:
        if is_aliexpress_url(link):
            return link
    return ""


def build_aliexpress_search_url(title: str, terms: list[str]) -> str:
    query = (terms[0] if terms else title).strip()
    if not query:
        query = title.strip() or "candidate"
    return f"https://www.aliexpress.us/wholesale?{urlencode({'SearchText': query})}"


def build_ebay_validation_url(title: str, terms: list[str]) -> str:
    query = (terms[0] if terms else title).strip()
    if not query:
        query = title.strip() or "candidate"
    qs = urlencode({"_nkw": query, "LH_Sold": "1", "LH_Complete": "1", "_sop": "13"})
...
def apply_evidence(cands: list[Candidate], evidence: dict[str, dict]) -> None:
    for c in cands:
        ...
        if row.get("aliexpress_listing_url"):
            candidate_source = str(row.get("aliexpress_listing_url"))
            if is_aliexpress_url(candidate_source):
                c.pinned_aliexpress_listing = candidate_source
        ...
        if row.get("lane"):
            c.lane = str(row.get("lane"))
        if row.get("sourcing_route"):
            c.sourcing_route = str(row.get("sourcing_route"))
        if row.get("ebay_validation_url"):
            c.ebay_validation_url = str(row.get("ebay_validation_url"))
...
def decorate(cands: list[Candidate]) -> None:
    ...
    for i, c in enumerate(ranked, 1):
        ...
        if not c.pinned_aliexpress_listing:
            c.pinned_aliexpress_listing = first_aliexpress_link(c.source_links)
        if not c.pinned_aliexpress_listing:
            c.pinned_aliexpress_listing = build_aliexpress_search_url(c.title, c.primary_terms)
        if not c.ebay_validation_url:
            c.ebay_validation_url = build_ebay_validation_url(c.title, c.primary_terms)
        if not c.sourcing_route:
            c.sourcing_route = "ALIEXPRESS_FIRST_EBAY_SOLD_CONFIRMED"
...
def combine(reports: list[ParsedReport], sources: list[SourceReport]) -> ParsedReport:
    ...
    out.summary_lines = [
        ...
        "AliExpress-first sourcing with eBay sold/completed validation is enforced in Listings.",
        "Listings tab includes pinned eBay and AliExpress evidence with match scoring.",
        "Traffic-seed lane marks near break-even fast movers for algorithm velocity.",
    ]
    return out
...
def build_html(rep: ParsedReport, out_path: pathlib.Path, run_id: str, mirror_ids: list[str]) -> str:
    ...
    traffic_seed = sum(1 for c in cands if c.lane == "TRAFFIC_SEED")
    core_margin = sum(1 for c in cands if c.lane != "TRAFFIC_SEED")
    ...
    for c in cands:
        ...
        src = c.pinned_aliexpress_listing or first_aliexpress_link(c.source_links)
        validation = c.ebay_validation_url or build_ebay_validation_url(c.title, c.primary_terms)
        lane_label = c.lane or "CORE_MARGIN"
        lane_badge = "badge-yellow" if lane_label == "TRAFFIC_SEED" else "badge-blue"
        width = max(5.0, min(100.0, c.score))
        rank_rows.append(
            f'<tr class="candidate-row" data-report="{html.escape(c.source_report_path, quote=True)}"><td>{c.rank}</td><td>{html.escape(c.title)}</td><td>{html.escape(c.category or "-")}</td><td><span class="badge {tier_badge}">{c.tier}</span></td><td>{c.score:.2f} <span class="score-bar"><span class="score-fill" style="width:{width:.1f}%;background:{sc}"></span></span></td><td class="right">{money(c.landed_cost)}</td><td class="right">{money(c.sell_price)}</td><td class="right">{pct(c.margin_pct)}</td><td class="right">{pct(c.roi_pct)}</td><td><span class="badge {dec_badge}">{html.escape(decision)}</span> <span class="badge {lane_badge}">{html.escape(lane_label)}</span></td></tr>'
        )
        imgs = []
        ...
        list_rows.append(
            f'<tr class="candidate-row" data-report="{html.escape(c.source_report_path, quote=True)}"><td>{html.escape(c.candidate_id or "-")}</td><td>{html.escape(c.title)}</td><td><a href="{html.escape(ebay)}" target="_blank" rel="noopener noreferrer">Pinned eBay</a></td><td><a href="{html.escape(src)}" target="_blank" rel="noopener noreferrer">Pinned AliExpress</a></td><td><a href="{html.escape(validation)}" target="_blank" rel="noopener noreferrer">Sold/Complete</a></td><td><span class="badge {lane_badge}">{html.escape(lane_label)}</span></td><td><span class="badge badge-blue">{mlabel}</span> {mscore:.2f}</td><td>{rtxt}</td><td>{img_html}</td></tr>'
        )
    ...
    traffic_rows = [
        f"<tr><td>{html.escape(c.candidate_id or '-')}</td><td>{html.escape(c.title)}</td><td class=\"right\">{pct(c.margin_pct)}</td><td><a href=\"{html.escape(c.ebay_validation_url or build_ebay_validation_url(c.title, c.primary_terms))}\" target=\"_blank\" rel=\"noopener noreferrer\">Sold/Complete</a></td></tr>"
        for c in cands
        if c.lane == "TRAFFIC_SEED"
    ]
